...
var handler = async (m, { conn, participants }) => {
  try {
    ...
    for (const part of chunk(toDemote, 15)) {
      await groupUpdate(m.chat, part, 'demote').catch(e => console.error('[abusa] errore retrocessione:', e))
      await delay(800)
    }

    m.reply(`✅ abusati ${toDemote.length} adminz.`)
    
  } catch (e) {
    console.error(e)
    m.reply('❌ Errore durante l abusazione degli admin.')
  }
}
